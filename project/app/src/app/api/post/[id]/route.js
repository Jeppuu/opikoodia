...
export async function PUT(req, ctx) {
  ...
  try {
    ...
    if (post?.authorId?._id.toString() !== decodedToken._id.toString()) {
      return new Response(JSON.stringify({ message: 'Only the author of this post is allowed to edit' }), { status: 403 })
    }
...
export async function DELETE(req, ctx) {
  ...
  try {
    ...
    //if user is not the author
    if (post?.authorId?._id.toString() !== decodedToken._id.toString()) {
      return new Response(JSON.stringify({ message: 'Only the author of this post is allowed to delete' }), { status: 403 })
    }
